...
def load_inference_model(model_path, args):
    return models.load_model(model_path, backbone_name=args) # args.backbone)
...
def postprocess_image(model_output, args):   
    boxes = model_output[0]
    scores = model_output[1] 
    labels = model_output[2] 
...
def preprocess_image(image, args):
    ...
    # loaded_image = keras_pre_image.img_to_array(image.convert('RGB'))
    open_image = Image.open(image)
    loaded_image = keras_pre_image.img_to_array(Image.open(image))
    preprocessed_image, scale = retinanet_image.resize_image(retinanet_image.preprocess_image(loaded_image[:, :, ::-1].copy()))
    loaded_image, scale = retinanet_image.resize_image( keras_pre_image.img_to_array(open_image.convert('RGB')))
